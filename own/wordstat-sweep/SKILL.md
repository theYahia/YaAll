...
> - SEO-ядро: `collect_semantics.py` живёт в `D:/Yahia/active/GosMaxCatalog/` и **в пакет не попал**. Скопировать его неоткуда. Шаг 3 «cp из эталона» на маке невыполним. Если пользователь просит семантическое ядро — сказать прямо, что скрипта нет, и предложить gate-режим либо ручной сбор через Wordstat-интерфейс.
